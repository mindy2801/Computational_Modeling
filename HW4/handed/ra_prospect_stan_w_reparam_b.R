...
myinits <- list(
  list(mu_p=rep(.1, 3), sigma_rho=.2, sigma_lambda=.6, sigma_tau=.9, 
       rho_p=rep(.1, N), lambda_p=rep(.1, N), tau_p=rep(.1,N)),
  list(mu_p=rep(.1, 3), sigma_rho=.2, sigma_lambda=.6, sigma_tau=.9, 
       rho_p=rep(.1, N), lambda_p=rep(.1, N), tau_p=rep(.1,N)))

# run!
output = stan("ra_prospect_w_reparam_b.stan", data = dataList, init = myinits,
...
load("ra_prospect_w_reparam_b.RData")
traceplot(output)
...
parameters$sigma <- data.frame(rho=parameters$sigma_rho, lambda=parameters$sigma_lambda, 
                               tau=parameters$sigma_tau)
...
g3 <- ggplot(group, aes(value, fill=variable)) + geom_histogram(bins = 50) + 
...
i3 <- ggplot(individual, aes(value, fill=variable)) + geom_histogram(bins = 50) + 
