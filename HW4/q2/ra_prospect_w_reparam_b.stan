...
transformed parameters {
  ...
  for (i in 1:N) {
    rho[i]    = Phi_approx( mu_p[1] + sigma_rho * rho_p[i] ) * 2;
    lambda[i] = Phi_approx( mu_p[2] + sigma_lambda * lambda_p[i] ) * 5; 
  }
  tau = exp( mu_p[3] + sigma_tau * tau_p );
}
model {
  // ra_prospect: Original model in Soko-Hessner et al 2009 PNAS
  // hyper parameters
  mu_p          ~ normal(0, 1.0);
  sigma_rho     ~ normal(0, 1.0);
  sigma_tau     ~ cauchy(0, 5.0);
  sigma_lambda  ~ cauchy(0, 5.0);
