...
g3_mu <- ggplot(mu_p, aes(value)) + geom_density() + 
  facet_wrap(~parameter, nrow=2) 

d <- ggplot_build(g3_mu)$data[[1]] 
...
i3_alpha <- ggplot(alpha, aes(value)) + geom_density() + 
  facet_wrap(~subject, ncol=20) 

d <- ggplot_build(i3_alpha)$data[[1]] 
d <- d %>% mutate(subject=rep(paste("subj",1:N, sep=""), each=nrow(d)/N)) %>% 
  select(x,y,subject) %>% 
  merge(alpha_HDI, by="subject") %>% group_by(subject) %>% 
  filter(x <= HDI2 & x >= HDI1)

i3_alpha <-i3_alpha + geom_area(data = d, 
                              aes(x=x, y=y), fill="#FF9999", colour="black", group="subject") + 
  geom_vline(data=alpha_HDI, aes(xintercept=mean)) + 
  xlab("") + theme_bw()

i3_beta <- ggplot(beta, aes(value)) + geom_density() + 
  facet_wrap(~subject, ncol=20) 

d <- ggplot_build(i3_beta)$data[[1]] 
d <- d %>% mutate(subject=rep(paste("subj",1:N, sep=""), each=nrow(d)/N)) %>% 
  select(x,y,subject) %>% 
  merge(beta_HDI, by="subject") %>% group_by(subject) %>% 
  filter(x <= HDI2 & x >= HDI1)

i3_beta <-i3_beta + geom_area(data = d, 
                            aes(x=x, y=y), fill="#FF9999", colour="black", group="subject") + 
...
c3 <- ggplot(pars_compare, aes(x=true, y=estm)) + geom_point(alpha=.5) + 
  facet_wrap(~parameter, scales = "free") +
  geom_abline(aes(intercept=0, slope=1, colour="red")) +
  geom_errorbar(aes(ymin=estm-sd, ymax=estm+sd), alpha=.5)

save(c3, g3_mu, g3_sigma, i3_alpha, i3_beta, file="graph3.RData")
